document.addEventListener("DOMContentLoaded", function() {
  ...
  n && i && n.addEventListener("click", function() {
      ...
      n.querySelector("ion-icon").classList.toggle("rotate")
  }),
  l && s && l.addEventListener("click", function() {
      s.classList.toggle("show1"),
      l.querySelector("ion-icon").classList.toggle("rotate")
  });
...
document.addEventListener("DOMContentLoaded", function() {
  ...
  e && e.addEventListener("click", function() {
      "none" === t.style.display || "" === t.style.display ? (t.style.display = "block", t.classList.add("open")) : (t.style.display = "none", t.classList.remove("open"))
  }),
  document.querySelectorAll(".options-list li").forEach(function(e) {
      e.addEventListener("click", function() {
          ...
          n.innerHTML = ` <img class="selected-icon"src="${s}"alt="${i}">${i}`, l.value = o,t.style.display = "none",
          t.classList.remove("open")
      })
  }),
  document.addEventListener("click", function(n) {
      e && !e.contains(n.target) && t && !t.contains(n.target) && (t.style.display = "none", t.classList.remove("open"))
  }),
  i && i.addEventListener("submit", function(e) {
      ...
      if (!n) {
          alert("Veuillez s\xe9lectionner votre type (particulier ou entreprise).");
          return
      }
      if (!i) {
      ...
      } else alert("Veuillez s\xe9lectionner une cat\xe9gorie de nuisible.")
  })
}),
document.addEventListener("DOMContentLoaded", function() {
  ...
  s = 0,
  o = parseFloat(document.querySelector(".cCarousel-item").getBoundingClientRect().width, 10) + parseFloat(window.getComputedStyle(l).getPropertyValue("gap"), 10);
  e.addEventListener("click", () => {
      0 !== s && (s += o, l.style.left = s + "px")
  }),
  t.addEventListener("click", () => {
      let e = n.getBoundingClientRect().width;
      i - Math.abs(s) > e && (s -= o, l.style.left = s + "px")
  });
  ...
  function u() {
      ...
      s <= -o && c < e ? (s += o, l.style.left = s + "px", c = e) : s <= -o && c > e && (s -= o, l.style.left = s + "px", c = e)
  }
}),
function(e) {
  e.fn.menumaker = function(t) {
      ...
      return this.each(function() {
          return e(this).find(".button").on("click", function() {
              ...
              t.hasClass("open") ? t.slideToggle().removeClass("open") : (t.slideToggle().addClass("open"), "dropdown" === l.format && t.find("ul").show())
          }),
          ...
          multiTg = function() {
              ...
              n.find(".submenu-button").on("click", function() {
                  ...
                  e(this).siblings("ul").hasClass("open") ? e(this).siblings("ul").removeClass("open").slideToggle() : e(this).siblings("ul").addClass("open").slideToggle()
              })
          },
          "multitoggle" === l.format ? multiTg() : n.addClass("dropdown"),
          !0 === l.sticky && n.css("position", "fixed"),
          (resizeFix = function() {
              e(window).width() > 1e3 && n.find("ul").show(),
              1e3 >= e(window).width() && n.find("ul").hide().removeClass("open")
          })(),
...
} (jQuery);document.getElementById("contactForm").addEventListener("submit", function(e) {
  e.preventDefault(),
  e = new FormData(this),
